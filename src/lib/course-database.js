...
async function initializeCourseTables() {
  try {
...
async function addVideoToCourse(videoData) {
  try {
    const id = uuidv4();
    const result = await sql`
...
async function updateCourseVideo(id, videoData) {
  try {
    const result = await sql`
...
// User access control
async function checkUserCourseAccess(userId, courseId) {
  try {
...
async function grantCourseAccess(userId, courseId, accessType, paymentTxHash, expiresAt) {
  try {
    const id = uuidv4();
    await sql`
